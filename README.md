...
* `monitor.sh`는 `pgrep`, `ps`, `date`, `sleep` 명령어를 이용하여 PID, CPU 사용률, 메모리 사용률을 주기적으로 수집하도록 구성하였다. `ps -p PID -o %cpu,%mem` 옵션을 사용해 필요한 데이터만 추출하였다.

* CPU 사용률 분석에는 `top`과 `ps`를 함께 사용하였다. `top`은 실시간 부하 변화를 확인하기에 적합하고, `ps`는 특정 프로세스의 CPU·메모리 사용률을 로그 형태로 저장하기에 적합하였다.

* 프로세스가 멈춘 상황에서는 먼저 `ps -ef`로 PID 존재 여부를 확인하고, 이후 `top -H`로 스레드별 상태를 확인하였다. PID는 존재하지만 CPU 사용률이 0%에 가까우며 로그가 더 이상 출력되지 않는 경우 Deadlock을 의심하였다.

* 메모리 누수 발생 시 프로세스를 강제 종료하는 이유는 시스템 전체 메모리 고갈을 방지하기 위함이다. 메모리가 모두 소진되면 다른 서비스까지 영향을 받아 시스템 전체 장애로 확산될 수 있다.

* CPU를 과도하게 사용하는 단일 프로세스를 제어하는 이유는 시스템 전체 응답성을 유지하기 위해서이다. 특정 프로세스가 CPU를 독점하면 다른 프로세스의 처리 지연과 시스템 성능 저하가 발생할 수 있다.

* Deadlock은 두 개 이상의 스레드가 서로가 가진 자원을 기다하면서 무한 대기 상태에 빠지는 현상이다. 이번 실습에서는 Thread-1이 `Shared_Memory_A`를 점유한 상태에서 `Socket_Pool_B`를 기다리고, Thread-2는 `Socket_Pool_B`를 점유한 상태에서 `Shared_Memory_A`를 기다하면서 순환 대기가 발생하였다.

* 실제 운영 환경에서는 `monitor.sh`에 RSS 메모리, 스레드 수, 로그 파일 저장 기능, 임계치 초과 시 알림 기능 등을 추가하여 장애를 더 빠르게 감지할 수 있도록 개선할 수 있다.

* 세 가지 장애 중 가장 치명적인 장애는 OOM이라고 판단하였다. 메모리 고갈은 단일 프로세스 장애를 넘어 다른 서비스와 시스템 전체 장애로 이어질 가능성이 크기 때문이다.

* OOM과 Deadlock이 동시에 의심되는 상황이라면 먼저 OOM 여부를 확인할 것이다. 프로세스가 종료되면 Deadlock 분석에 필요한 증거가 사라지기 때문에 메모리 관련 증거를 우선 수집하는 것이 중요하다.

* 코드 레벨에서는 OOM의 경우 메모리 해제 로직 추가, CPU Spike의 경우 Busy Waiting 제거와 알고리즘 개선, Deadlock의 경우 Lock 순서 통일 및 Timed Mutex 적용과 같은 방법으로 개선할 수 있다.

* 미션을 다시 수행한다면 장애를 재현하기 전에 먼저 모니터링 스크립트와 로그 수집 환경을 구축할 것이다. 이후 환경변수를 하나씩 변경하며 데이터를 축적하고, 충분한 증거를 확보한 뒤 원인을 분석하는 방식으로 접근할 것이다.
